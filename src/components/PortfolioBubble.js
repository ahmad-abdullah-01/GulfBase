...
import { ChartView, bubbleChart } from './charts';
import { usePortfolioWidget, usePortfolioDashboardData } from '../hooks/usePortfolios';

export default function PortfolioBubble({ activePortfolio }) {
    ...
    const filteredChartData = dashboard?.chartData?.map(item => ({
        x: item.x,
        y: item.y,
        z: item.z,
        name: item.name
    ...
    return (
        <View style={styles.container}>
            ...
            {dashboardLoading ? (
                ...
                <ChartView config={bubbleChart(filteredChartData)} />
            )}
        </View>
    );
}
