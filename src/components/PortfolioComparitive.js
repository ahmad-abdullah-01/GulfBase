...
export default function PortfolioComparitive({ activePortfolio }) {
    ...
    const categories = dashboard?.chartData?.map(item => item.name);
    const value = dashboard?.chartData?.map(item => item.y);
    ...
    return (
        <View style={styles.container}>
            ...
            {dashboardLoading ? (
                ...
                <ChartView config={barChart(categories, value)} />
            )}
        </View>
    );
}
